...
async function ensureReferences(){if(referencesLoaded)return;const [teams,members,venues,levels,seasons,tournaments,entries,shopCategories]=await Promise.all(['teams','club_members','venues','competition_levels','seasons','tournaments','team_competitions','shop_categories'].map(async resource=>{const response=await fetch(`/admin-api/${resource}`,{headers:authHeaders()});if(!response.ok)throw new Error(response.status===401?'Session Cloudflare expirée. Rechargez la page.':'Données de référence indisponibles.');return response.json()}));references={teams,club_members:members,venues,competition_levels:levels,seasons,tournaments,team_competitions:entries,shop_categories:shopCategories};referencesLoaded=true;}
async function load(){
  $('#new').hidden=['team_competitions','site_media','shop_settings'].includes(current);
...
function field(name,value){
  ...
  if(files.has(name)){const imageField=['photo_key','logo_key','image_key'].includes(name)||(current==='site_media'&&name==='object_key');return `<label class="file-field"><span class="field-title">${title}</span><input type="hidden" name="${name}" value="${esc(value)}"><input type="file" data-upload="${name}" data-image="${imageField?'1':'0'}" accept="${imageField?'image/*':'.pdf'}"><span class="file-state">${value?`Fichier actuel : ${esc(value)}`:'Choisir un fichier'}</span>${value&&imageField?`<img src="/media/${esc(value)}" alt="Aperçu">`:value?`<a href="/media/${esc(value)}" target="_blank" rel="noopener">Ouvrir le fichier actuel</a>`:''}</label>`}
