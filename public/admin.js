...
async function load(){
  ...
  if(['team_staff','training_sessions','teams','team_competitions','tournament_teams','tournaments','matches'].includes(current)){try{await ensureReferences()}catch(error){$('#status').textContent=error.message;$('#records').innerHTML='';return}}
