...
fetch('/api/page/tournaments').then(response=>response.json()).then(data=>{
  tournaments=data.tournaments||[];links=data.links||[];teams=(data.teams||[]).slice().sort((a,b)=>String(a.name).localeCompare(String(b.name),'fr',{numeric:true,sensitivity:'base'}));venues=data.venues||[];
