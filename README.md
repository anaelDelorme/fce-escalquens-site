...
La clé ZenRows reste uniquement dans les secrets GitHub : elle ne doit être ajoutée ni au dépôt, ni à Cloudflare, ni à `.dev.vars`. Après le déploiement, lancez Actions → « Synchroniser les matchs » → Run workflow. Le journal attendu commence par `Collecteur FCE 2026.09.02-10`, puis affiche `FFF : … matchs et … plateaux reçus via zenrows-browser.`. Une seule requête ZenRows doit être comptabilisée pour cette exécution.
